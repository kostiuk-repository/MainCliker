...
public class WallTower extends BaseJobController implements View.OnTouchListener{


    public static final String ID = "WallTower";
    private int onMode = R.mipmap.wall_tower_on;
    private int offMode = R.mipmap.wall_tower_off;
    private int help = R.mipmap.wall_tower_craft;
    private BaseJobController baseJobController;
    private static WallTower instance;
    ...
    public static synchronized WallTower getInstance(Context context) {
        if (instance == null) {
            instance = new WallTower(context);
        }
        return instance;
    }


    private WallTower(Context context) {
        super(context);
    ...
    public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(Cobbl.getInstance(context).getIT().getViewCounter()>=25
                        && VillagerGold.getInstance(context).getIT().getViewCounter()>=1) {
                    changeStateIfClick();
