...
public class Zabor extends BaseJobController implements View.OnTouchListener{


    public static final String ID = "ZABOR";
    private int onMode = R.mipmap.zabor01_on;
    private int offMode = R.mipmap.zabor01_off;
    private int help = R.mipmap.zabor01_craft;
    private BaseJobController baseJobController;
    private static Zabor instance;
    ...
    public static synchronized Zabor getInstance(Context context) {
        if (instance == null) {
            instance = new Zabor(context);
        }
        return instance;
    }


    private Zabor(Context context) {
        super(context);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(Crafttable.getInstance(context).getIT().getViewCounter()>=1
                        && PlankButton.getInstance(context).getIT().getViewCounter()>=4
                        && Stick.getInstance(context).getIT().getViewCounter()>=2) {
                    changeStateIfClick();
                    ZaborCopy.getInstance(context).getIT().setViewCounter(baseJobController.getViewCounter());
                    ZaborCopy.getInstance(context).getIT().lockUnlockFunction(ZaborCopy.getInstance(context).getIT());
                    ZaborCopy.getInstance(context).getIT().resolveTextViewPosition();
                    ZaborCopy.getInstance(context).getIT().changeStateIfCheck();
