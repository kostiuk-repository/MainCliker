...
public class MineWorker extends BaseJobController implements View.OnTouchListener{


    public static final String ID = "VILLAGER_MINE";
    private int onMode = R.mipmap.villager_mine_fire_on;
    private int offMode = R.mipmap.villager_mine_fire_off;
    private int help = R.mipmap.villager_mine_fire_craft;
    private BaseJobController baseJobController;
    private static MineWorker instance;
    ...
    public static synchronized MineWorker getInstance(Context context) {
        if (instance == null) {
            instance = new MineWorker(context);
        }
        return instance;
    }


    private MineWorker(Context context) {
        super(context);
        ...
        baseJobController = new BaseJobController.Builder(context)
                ...
                .setOff(offMode)
                .setHelp(help)
                .setCompareList(25,25,25)
                .setStatuses(STATUS.MINUS,STATUS.MINUS,STATUS.MINUS)
                .setMinusValueList(-25,-25,-25)
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(upBarLeftSide.getTextFieldCounter() < 150
                        ...
                        && StonePickaxe.getInstance(context).getIT().getViewCounter()>=25) {
                    changeStateIfClick();
                    baseJobController.animateOnes(baseJobController);
                    if(baseJobController.getIT().getViewCounter()<=1) {
                        disposable = mine.getWorker().subscribe();
                    }
