...
public class Lead extends BaseJobController implements View.OnTouchListener{


    public static final String ID = "Lead";
    private int onMode = R.mipmap.lead_on;
    private int offMode = R.mipmap.lead_off;
    private int help = R.mipmap.lead_craft;
    ...
    public static synchronized Lead getInstance(Context context) {
        if (instance == null) {
            instance = new Lead(context);
        }
        return instance;
    }


    private Lead(Context context) {
        super(context);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(VillagerHorseTrener.getInstance(context).getIT().getViewCounter()>=1
                            && Slime.getInstance(context).getIT().getViewCounter()>=1
                        && Sttring.getInstance(context).getIT().getViewCounter()>=1) {
                    changeStateIfClick();
