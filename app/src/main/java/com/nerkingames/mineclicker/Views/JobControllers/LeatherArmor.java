...
public class LeatherArmor extends BaseJobController implements View.OnTouchListener{


    public static final String ID = "LEATHER_ARMOR";
    private int onMode = R.mipmap.leather_armor_on;
    private int offMode = R.mipmap.leather_armor_off;
    private int help = R.mipmap.leather_armor_craft;
    private BaseJobController baseJobController;
    private static LeatherArmor instance;
    ...
    public static synchronized LeatherArmor getInstance(Context context) {
        if (instance == null) {
            instance = new LeatherArmor(context);
        }
        return instance;
    }


    private LeatherArmor(Context context) {
        super(context);
    ...
    public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(LeatherHelmet.getInstance(context).getIT().getViewCounter()>=1
                        && LeatherChestplate.getInstance(context).getIT().getViewCounter()>=1
                        && LeatherBoots.getInstance(context).getIT().getViewCounter()>=1
                        && LeatherLegins.getInstance(context).getIT().getViewCounter()>=1) {
                    changeStateIfClick();
