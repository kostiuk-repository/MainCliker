...
public class Ladder extends BaseJobController implements View.OnTouchListener{


    public static final String ID = "LADDER";
    private int onMode = R.mipmap.ladder_on;
    private int offMode = R.mipmap.ladder_off;
    private int help = R.mipmap.stairs0_craft;
    ...
    public static synchronized Ladder getInstance(Context context) {
        if (instance == null) {
            instance = new Ladder(context);
        }
        return instance;
    }


    private Ladder(Context context) {
        super(context);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(Crafttable.getInstance(context).getIT().getViewCounter()>=1
                        && Stick.getInstance(context).getIT().getViewCounter()>=7) {
                    changeStateIfClick();
