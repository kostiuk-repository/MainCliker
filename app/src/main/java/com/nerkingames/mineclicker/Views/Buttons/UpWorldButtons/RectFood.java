...
public class RectFood extends View {
    ...
    public RectFood(Context context, IViewGetter food) {
        super(context);
    ...
    private void initRectangle() {
        ...
        wheat = Wheat.getInstance(context);
        bambuk = Bambuk.getInstance(context);
    }
    ...
    public void clearResource() {
        handler.post(()->{
            thisView.removeAllViews();
            frameLayoutProgressBar.removeAllViews();
            if (FoodWorker.getInstance(context).getIT().getViewCounter()>0){
                if(!GameActivity.onStop) {
                    food.setDisposable(food.getWorker().subscribe());
                }
    ...
    public void increaseTheCounter(int period, int type){
        ...
        observable.subscribe(new Observer<Long>() {
            ...
            public void onComplete() {
                ...
                food.setClicked(false);
                if(type == 1) {
                    wheat.subscribe(Observable.just(Integer.valueOf(1)));
                } else if (type == 2) {
                    bambuk.subscribe(Observable.just(Integer.valueOf(1)));
                }
            }
        });
    }
