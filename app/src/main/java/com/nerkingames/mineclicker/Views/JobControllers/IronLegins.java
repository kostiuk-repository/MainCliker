...
public class IronLegins extends BaseJobController implements View.OnTouchListener{


    public static final String ID = "IRON_LEGINS";
    private int onMode = R.mipmap.iron_legins_on;
    private int offMode = R.mipmap.iron_legins_off;
    private int help = R.mipmap.iron_legins_craft;
    private BaseJobController baseJobController;
    private static IronLegins instance;
    ...
    public static synchronized IronLegins getInstance(Context context) {
        if (instance == null) {
            instance = new IronLegins(context);
        }
        return instance;
    }


    private IronLegins(Context context) {
        super(context);
    ...
    public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(VillagerAutosmeltUpdte.getInstance(context).getIT().getViewCounter()>=1
                        && IronIngot.getInstance(context).getIT().getViewCounter()>=7) {
                    changeStateIfClick();
