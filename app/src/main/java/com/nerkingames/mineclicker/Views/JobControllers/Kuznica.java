...
public class Kuznica extends BaseJobController implements View.OnTouchListener{


    public static final String ID = "KUZNICA";
    private int onMode = R.mipmap.kuznica_on;
    private int offMode = R.mipmap.kuznica_off;
    private int help = R.mipmap.kuznica_craft;
    private BaseJobController baseJobController;
    private static Kuznica instance;
    ...
    public static synchronized Kuznica getInstance(Context context) {
        if (instance == null) {
            instance = new Kuznica(context);
        }
        return instance;
    }


    private Kuznica(Context context) {
        super(context);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(Home.getInstance(context).getIT().getViewCounter()>=2
                        && LavaPool.getInstance(context).getIT().getViewCounter()>=1
                        && Furnece.getInstance(context).getIT().getViewCounter()>=1) {
                    changeStateIfClick();
