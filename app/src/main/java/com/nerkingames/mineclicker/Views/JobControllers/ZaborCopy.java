...
public class ZaborCopy extends BaseJobController implements View.OnTouchListener{


    public static final String ID = "ZaborCopy";
    private int onMode = R.mipmap.zabor01_on;
    private int offMode = R.mipmap.zabor01_off;
    private int help = R.mipmap.zabor01_01_craft;
    private BaseJobController baseJobController;
    private static ZaborCopy instance;
    ...
    public static synchronized ZaborCopy getInstance(Context context) {
        if (instance == null) {
            instance = new ZaborCopy(context);
        }
        return instance;
    }


    private ZaborCopy(Context context) {
        super(context);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(VillagerSaw.getInstance(context).getIT().getViewCounter()>=1
                        && WoodButton.getInstance(context).getIT().getViewCounter()>=2) {
                    changeStateIfClick();
                    Zabor.getInstance(context).getIT().setViewCounter(baseJobController.getViewCounter());
                    Zabor.getInstance(context).getIT().lockUnlockFunction(Zabor.getInstance(context).getIT());
                    Zabor.getInstance(context).getIT().resolveTextViewPosition();
                    Zabor.getInstance(context).getIT().changeStateIfCheck();
