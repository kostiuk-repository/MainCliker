...
public class VillagerLetherSoldat extends BaseJobController implements View.OnTouchListener{


    public static final String ID = "VILLAGER_LEATHER_SOLDAT";
    private int onMode = R.mipmap.villager_lether_soldat_on;
    private int offMode = R.mipmap.villager_lether_soldat_off;
    private int help = R.mipmap.villager_lether_soldat_craft;
    private BaseJobController baseJobController;
    private static VillagerLetherSoldat instance;
    ...
    public static synchronized VillagerLetherSoldat getInstance(Context context) {
        if (instance == null) {
            instance = new VillagerLetherSoldat(context);
        }
        return instance;
    }


    private VillagerLetherSoldat(Context context) {
        super(context);
        ...
        baseJobController = new BaseJobController.Builder(context)
                ...
                .setOff(offMode)
                .setHelp(help)
                .setCompareList(1,1,1,0,0)
                .setStatuses(STATUS.MINUS,STATUS.MINUS,STATUS.MINUS,STATUS.NOTHING,STATUS.NOTHING)
                .setMinusValueList(-1,-1,-1,0,0)
    ...
    public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(VillagerSoldat.getInstance(context).getIT().getViewCounter()>=1
                        && LeatherArmor.getInstance(context).getIT().getViewCounter()>=1) {
                    changeStateIfClick();
