...
public class VillagerIronSoldat extends BaseJobController implements View.OnTouchListener{




    public static final String ID = "VILLAGER_IRON_SOLDAT";
    private int onMode = R.mipmap.villager_iron_soldat_on;
    private int offMode = R.mipmap.villager_iron_soldat_off;
    private int help = R.mipmap.villager_iron_soldat_craft;
    private BaseJobController baseJobController;
    private static VillagerIronSoldat instance;
    ...
    public static synchronized VillagerIronSoldat getInstance(Context context) {
        if (instance == null) {
            instance = new VillagerIronSoldat(context);
        }
        return instance;
    }


    private VillagerIronSoldat(Context context) {
        super(context);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(VillagerSoldat.getInstance(context).getIT().getViewCounter()>=1
                        && IronArmor.getInstance(context).getIT().getViewCounter()>=1) {
                    changeStateIfClick();
