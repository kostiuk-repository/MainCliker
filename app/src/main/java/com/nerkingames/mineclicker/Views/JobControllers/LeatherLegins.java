...
public class LeatherLegins extends BaseJobController implements View.OnTouchListener{


    public static final String ID = "LEATHER_LEGINS";
    private int onMode = R.mipmap.leather_legins_on;
    private int offMode = R.mipmap.leather_legins_off;
    private int help = R.mipmap.leather_legins_craft;
    private BaseJobController baseJobController;
    private static LeatherLegins instance;
    ...
    public static synchronized LeatherLegins getInstance(Context context) {
        if (instance == null) {
            instance = new LeatherLegins(context);
        }
        return instance;
    }


    private LeatherLegins(Context context) {
        super(context);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(VillagerLeather.getInstance(context).getIT().getViewCounter()>=1
                        && Leather.getInstance(context).getIT().getViewCounter()>=7) {
                    changeStateIfClick();
