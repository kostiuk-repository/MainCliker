...
public class Stoilo extends BaseJobController implements View.OnTouchListener{


    public static final String ID = "Stoilo";
    private int onMode = R.mipmap.stoilo_on;
    private int offMode = R.mipmap.stoilo_off;
    private int help = R.mipmap.stoilo_craft;
    private BaseJobController baseJobController;
    private static Stoilo instance;
    ...
    public static synchronized Stoilo getInstance(Context context) {
        if (instance == null) {
            instance = new Stoilo(context);
        }
        return instance;
    }


    private Stoilo(Context context) {
        super(context);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(VillagerSaw.getInstance(context).getIT().getViewCounter()>=1
                        && ZaborUpdate.getInstance(context).getIT().getViewCounter()>=1
                        && Roof.getInstance(context).getIT().getViewCounter()>=1
                        && WheatBlock.getInstance(context).getIT().getViewCounter()>=1) {
                    changeStateIfClick();
