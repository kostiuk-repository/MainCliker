...
public class DirtPodzol extends BaseJobController implements View.OnTouchListener{


    public static final String ID = "DIRT_PODZOL";
    private final Context context;
    private int onMode = R.mipmap.dirt_podzol_on;
    private int offMode = R.mipmap.dirt_podzol_off;
    private int help = R.mipmap.dirt_podzol_craft;
    private BaseJobController baseJobController;
    private static DirtPodzol instance;
    ...
    public static synchronized DirtPodzol getInstance(Context context) {
        if (instance == null) {
            instance = new DirtPodzol(context);
        }
        return instance;
    }


    private DirtPodzol(Context context) {
        super(context);
    ...
    public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(WoodHoe.getInstance(context).getIT().getViewCounter()>=1
                        && Dirt.getInstance(context).getIT().getViewCounter()>=1) {
                    changeStateIfClick();
