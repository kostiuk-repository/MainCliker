...
public class LavaPool extends BaseJobController implements View.OnTouchListener{


    public static final String ID = "LAVA_POOL";
    private int onMode = R.mipmap.lava_pool_on;
    private int offMode = R.mipmap.lava_pool_off;
    private int help = R.mipmap.lava_pool_craft;
    private BaseJobController baseJobController;
    private static LavaPool instance;
    ...
    public static synchronized LavaPool getInstance(Context context) {
        if (instance == null) {
            instance = new LavaPool(context);
        }
        return instance;
    }


    private LavaPool(Context context) {
        super(context);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(VillagerStroitel.getInstance(context).getIT().getViewCounter()>=1
                        && Cobbl.getInstance(context).getIT().getViewCounter()>=12
                        && Lava.getInstance(context).getIT().getViewCounter()>=4) {
                    changeStateIfClick();
