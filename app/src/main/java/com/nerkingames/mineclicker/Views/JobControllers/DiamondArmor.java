...
public class DiamondArmor extends BaseJobController implements View.OnTouchListener{


    public static final String ID = "DiamondArmor";
    private int onMode = R.mipmap.diamond_armor_on;
    private int offMode = R.mipmap.diamond_armor_off;
    private int help = R.mipmap.diamond_armor_craft;
    private BaseJobController baseJobController;
    private static DiamondArmor instance;
    ...
    public static synchronized DiamondArmor getInstance(Context context) {
        if (instance == null) {
            instance = new DiamondArmor(context);
        }
        return instance;
    }


    private DiamondArmor(Context context) {
        super(context);
    ...
    public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(VillagerWorkDiamond.getInstance(context).getIT().getViewCounter()>=1
                        && Diamond.getInstance(context).getIT().getViewCounter()>=34) {
                    changeStateIfClick();
