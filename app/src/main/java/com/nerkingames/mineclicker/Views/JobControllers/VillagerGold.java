...
public class VillagerGold extends BaseJobController implements View.OnTouchListener{


    public static final String ID = "VillagerGold";
    private int onMode = R.mipmap.villager_gold_on;
    private int offMode = R.mipmap.villager_gold_off;
    private int help = R.mipmap.villager_gold_craft;
    private BaseJobController baseJobController;
    private static VillagerGold instance;
    ...
    public static synchronized VillagerGold getInstance(Context context) {
        if (instance == null) {
            instance = new VillagerGold(context);
        }
        return instance;
    }


    private VillagerGold(Context context) {
        super(context);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(VillagerSoldat.getInstance(context).getIT().getViewCounter()>=1
                        && GoldArmor.getInstance(context).getIT().getViewCounter()>=1) {
                    changeStateIfClick();
