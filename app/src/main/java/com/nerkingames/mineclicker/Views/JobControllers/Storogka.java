...
public class Storogka extends BaseJobController implements View.OnTouchListener{


    public static final String ID = "STOROGKA";
    private int onMode = R.mipmap.storogka_on;
    private int offMode = R.mipmap.storogka_off;
    private int help = R.mipmap.storogka_craft;
    private BaseJobController baseJobController;
    private static Storogka instance;
    ...
    public static synchronized Storogka getInstance(Context context) {
        if (instance == null) {
            instance = new Storogka(context);
        }
        return instance;
    }


    private Storogka(Context context) {
        super(context);
        ...
        baseJobController = new BaseJobController.Builder(context)
                ...
                .setOff(offMode)
                .setHelp(help)
                .setCompareList(1,5,1,0,0,0,0,0,0,0,0)
                .setStatuses(STATUS.NOTHING,STATUS.MINUS,STATUS.MINUS,STATUS.NOTHING,STATUS.NOTHING,
                        ...
                        ,STATUS.NOTHING)
                .setMinusValueList(0,-5,-1,0,0,0,0,0,0,0,0)
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(Crafttable.getInstance(context).getIT().getViewCounter()>=1
                        && Wall.getInstance(context).getIT().getViewCounter()>=5
                        && Door.getInstance(context).getIT().getViewCounter()>=1) {
                    changeStateIfClick();
                    StorogkaAuto.getInstance(context).getIT().setViewCounter(baseJobController.getViewCounter());
                    StorogkaAuto.getInstance(context).getIT().lockUnlockFunction(StorogkaAuto.getInstance(context).getIT());
                    StorogkaAuto.getInstance(context).getIT().resolveTextViewPosition();
                    StorogkaAuto.getInstance(context).getIT().changeStateIfCheck();
