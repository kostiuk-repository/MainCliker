...
public class VillagerFermer extends BaseJobController implements View.OnTouchListener{


    public static final String ID = "VILLAGER_FERMER";
    private int onMode = R.mipmap.villager_fermer_on;
    private int offMode = R.mipmap.villager_fermer_off;
    private int help = R.mipmap.villager_fermer_craft;
    private BaseJobController baseJobController;
    private static VillagerFermer instance;
    ...
    public static synchronized VillagerFermer getInstance(Context context) {
        if (instance == null) {
            instance = new VillagerFermer(context);
        }
        return instance;
    }


    private VillagerFermer(Context context) {
        super(context);
    ...
    public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(EducatedVillager.getInstance(context).getIT().getViewCounter()>=1
                        && Pitchwork.getInstance(context).getIT().getViewCounter()>=1) {
                    changeStateIfClick();
