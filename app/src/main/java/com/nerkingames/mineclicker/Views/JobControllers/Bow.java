...
public class Bow extends BaseJobController implements View.OnTouchListener{


    public static final String ID = "BOW";
    private int onMode = R.mipmap.bow_on;
    private int offMode = R.mipmap.bow_off;
    private int help = R.mipmap.bow_craft;
    ...
    public static synchronized Bow getInstance(Context context) {
        if (instance == null) {
            instance = new Bow(context);
        }
        return instance;
    }
    ...
    public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(VillagerArowmaker.getInstance(context).getIT().getViewCounter()>=1
                        && Stick.getInstance(context).getIT().getViewCounter()>=3
                        && Sttring.getInstance(context).getIT().getViewCounter()>=3) {
                    changeStateIfClick();
