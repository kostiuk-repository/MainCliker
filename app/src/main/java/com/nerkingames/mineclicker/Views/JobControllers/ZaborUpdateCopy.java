...
public class ZaborUpdateCopy extends BaseJobController implements View.OnTouchListener{


    public static final String ID = "ZaborUpdateCopy";
    private int onMode = R.mipmap.zabor_on;
    private int offMode = R.mipmap.zabor_off;
    private int help = R.mipmap.zabor_01_craft;
    private BaseJobController baseJobController;
    private static ZaborUpdateCopy instance;
    ...
    public static synchronized ZaborUpdateCopy getInstance(Context context) {
        if (instance == null) {
            instance = new ZaborUpdateCopy(context);
        }
        return instance;
    }


    private ZaborUpdateCopy(Context context) {
        super(context);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(VillagerSaw.getInstance(context).getIT().getViewCounter()>=1
                    ...
                    changeStateIfClick();
                    ZaborUpdate.getInstance(context).getIT().setViewCounter(baseJobController.getViewCounter());
                    ZaborUpdate.getInstance(context).getIT().lockUnlockFunction(ZaborUpdate.getInstance(context).getIT());
                    ZaborUpdate.getInstance(context).getIT().resolveTextViewPosition();
                    ZaborUpdate.getInstance(context).getIT().changeStateIfCheck();
