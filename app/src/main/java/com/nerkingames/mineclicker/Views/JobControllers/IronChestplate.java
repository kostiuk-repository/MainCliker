...
public class IronChestplate extends BaseJobController implements View.OnTouchListener{


    public static final String ID = "IRON_CHESTPLATE";
    private int onMode = R.mipmap.iron_chestplate_on;
    private int offMode = R.mipmap.iron_chestplate_off;
    private int help = R.mipmap.iron_chestplate_craft;
    private BaseJobController baseJobController;
    private static IronChestplate instance;
    ...
    public static synchronized IronChestplate getInstance(Context context) {
        if (instance == null) {
            instance = new IronChestplate(context);
        }
        return instance;
    }


    private IronChestplate(Context context) {
        super(context);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(VillagerAutosmeltUpdte.getInstance(context).getIT().getViewCounter()>=1
                        && IronIngot.getInstance(context).getIT().getViewCounter()>=8) {
                    changeStateIfClick();
