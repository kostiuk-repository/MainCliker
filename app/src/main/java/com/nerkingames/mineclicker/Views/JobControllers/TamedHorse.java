...
public class TamedHorse extends BaseJobController implements View.OnTouchListener{


    public static final String ID = "TamedHorse";
    private int onMode = R.mipmap.tamed_horse_on;
    private int offMode = R.mipmap.tamed_horse_off;
    private int help = R.mipmap.tamed_horse_craft;
    private BaseJobController baseJobController;
    private static TamedHorse instance;
    ...
    public static synchronized TamedHorse getInstance(Context context) {
        if (instance == null) {
            instance = new TamedHorse(context);
        }
        return instance;
    }


    private TamedHorse(Context context) {
        super(context);
    ...
    public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(VillagerHorseTrener.getInstance(context).getIT().getViewCounter()>=1
                        && Horse.getInstance(context).getIT().getViewCounter()>=1
                        && Saddle.getInstance(context).getIT().getViewCounter()>=1) {
                    changeStateIfClick();
