...
public class IronArmor extends BaseJobController implements View.OnTouchListener{


    public static final String ID = "IRON_ARMOR";
    private int onMode = R.mipmap.iron_armor_on;
    private int offMode = R.mipmap.iron_armor_off;
    private int help = R.mipmap.iron_armor_craft;
    private BaseJobController baseJobController;
    private static IronArmor instance;
    ...
    public static synchronized IronArmor getInstance(Context context) {
        if (instance == null) {
            instance = new IronArmor(context);
        }
        return instance;
    }


    private IronArmor(Context context) {
        super(context);
    ...
    public boolean onTouch(View v, MotionEvent event) {
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(IronHelmet.getInstance(context).getIT().getViewCounter()>=1
                        && IronChestplate.getInstance(context).getIT().getViewCounter()>=1
                        && IronBoots.getInstance(context).getIT().getViewCounter()>=1
                        && IronLegins.getInstance(context).getIT().getViewCounter()>=1) {
                    changeStateIfClick();
