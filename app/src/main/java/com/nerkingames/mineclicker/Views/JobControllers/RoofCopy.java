...
public class RoofCopy extends BaseJobController implements View.OnTouchListener{


    public static final String ID = "RoofCopy";
    private int onMode = R.mipmap.krisha_on;
    private int offMode = R.mipmap.krisha_off;
    private int help = R.mipmap.krisha_01_craft;
    private BaseJobController baseJobController;
    private static RoofCopy instance;
    ...
    public static synchronized RoofCopy getInstance(Context context) {
        if (instance == null) {
            instance = new RoofCopy(context);
        }
        return instance;
    }


    private RoofCopy(Context context) {
        super(context);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(VillagerSaw.getInstance(context).getIT().getViewCounter()>=1
                        && TwoTimeHalfRoof.getInstance(context).getIT().getViewCounter()>=4) {
                    changeStateIfClick();
                    Roof.getInstance(context).getIT().setViewCounter(baseJobController.getViewCounter());
                    Roof.getInstance(context).getIT().lockUnlockFunction(Roof.getInstance(context).getIT());
                    Roof.getInstance(context).getIT().resolveTextViewPosition();
                    Roof.getInstance(context).getIT().changeStateIfCheck();
