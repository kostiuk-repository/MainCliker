...
public class TowerUpdate extends BaseJobController implements View.OnTouchListener{


    public static final String ID = "TOWER_UPDATE";
    private int onMode = R.mipmap.tower02_on;
    private int offMode = R.mipmap.tower02_off;
    private int help = R.mipmap.zastava_craft;
    private BaseJobController baseJobController;
    private static TowerUpdate instance;
    ...
    public static synchronized TowerUpdate getInstance(Context context) {
        if (instance == null) {
            instance = new TowerUpdate(context);
        }
        return instance;
    }


    private TowerUpdate(Context context) {
        super(context);
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if(VillagerStroitel.getInstance(context).getIT().getViewCounter()>=1
                        && StorogkaUpdate.getInstance(context).getIT().getViewCounter()>=1
                        && TowerAttak.getInstance(context).getIT().getViewCounter()>=1) {
                    changeStateIfClick();
